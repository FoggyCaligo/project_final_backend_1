...
class RecipeControllerTest {
    ...
    class GetRecipeDetail {
        ...
        void 회원_상세조회() throws Exception {
            ...
            mockMvc.perform(get("/api/v1/recipes/1").param("userId", "10"))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.success").value(true))
                    .andExpect(jsonPath("$.data.title").value("김치찌개"));
        }
    ...
    // ========================================================================
    // GET /api/v1/recipes/{recipeId}/cooked - 레시피 조리 완료 테스트
    // ========================================================================
    @Nested
    @DisplayName("GET /api/v1/recipes/{recipeId}/cooked - 레시피 조리 완료")
    class AteRecipe {
        ...
        void 조리완료_성공() throws Exception {
            ...
            // when & then
            mockMvc.perform(get("/api/v1/recipes/1/cooked").param("userId", "10"))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.success").value(true))
