...
public class CoupangShoppingClient {
    ...
    public List<ShoppingItemDto> search(String keyword) {
        if (accessKey.isBlank() || secretKey.isBlank()) {
            log.warn("[CoupangShoppingClient] API 키 미설정, 건너뜀");
        ...
        try {
            ...
            log.warn("[CoupangShoppingClient] 검색 실패 keyword={}: {}", keyword, e.getMessage());
            return Collections.emptyList();
        } catch (Exception e) {
            log.warn("[CoupangShoppingClient] HMAC 서명 실패: {}", e.getMessage());
            return Collections.emptyList();
        }
    }
