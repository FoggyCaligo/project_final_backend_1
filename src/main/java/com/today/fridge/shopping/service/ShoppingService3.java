...
public class ShoppingService3 {
    ...
    public IngredientPriceResponse getIngredientPrices(Long ingredientMasterId) {
         ...
         * opsForValue() : Redis 의 여러 데이터 구조 중 가장 기본인 String(Key-value) 형태를 다룬다, 키에 해당하는 값을 가져오거나 저장할때 사용
         ...
         */
        try { 
            // cashed 는 PRICE_PREFIX (shopping:price)에 식재료id를 붙인 키를 Redis 에서 조회 한 결과를 받아서 변수에 저장 
...
//  DB 를 조회하지 못하면 '식재료 마스터 정보를 찾을 수 없습니다.' 문구 출력 
        ...
        //  dbCached는 shoppingItem 테이블에서 식재료 id 기준으로 데이터를 조회해 가격순서대로 db를 조회한 결과를 받아서 변수에 저장 
    ...
    // ShoppingItem 은 식재료마스터DB에 mallName(네이버나 11번가 등), mallProductId, 구메아이템이름, 브랜드, 할인가격, 원래 가격, 할인비율, 이미지url, 쇼핑타입, 재고현황, 배송료, 리뷰평점, 받아온 시간, 만료시간, 업데이트 시간의 정보를 담는다.
    ...
    public IngredientPriceResponse searchByKeyword(String keyword) {
        ...
        // 2. 네이버 + 쿠팡 병렬 API 호출
        CompletableFuture<List<ShoppingItemDto>> naverFuture =
                CompletableFuture.supplyAsync(() -> naverClient.search(normalizedKeyword));
        CompletableFuture<List<ShoppingItemDto>> coupangFuture =
                CompletableFuture.supplyAsync(() -> coupangClient.search(normalizedKeyword));
        ...
        int lowestPrice = allItems.stream().mapToInt(ShoppingItemDto::getPrice).min().orElse(0);
        Instant now = Instant.now();
